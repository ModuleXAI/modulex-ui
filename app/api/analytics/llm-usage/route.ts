...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendUrl = new URL(`${modulexServerUrl}/dashboard/analytics/llm-usage`)
    ...
    if (!response.ok) {
      const message = await getApiErrorMessage(response, 'Failed to load LLM usage analytics')
