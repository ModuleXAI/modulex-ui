...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendUrl = new URL(`${modulexServerUrl}/integrations/service-keys/allowed`)
    ...
    if (!response.ok) {
      const message = await getApiErrorMessage(response, 'Failed to fetch allowed service keys')
