...
// Create or update a service key
export async function POST(request: NextRequest) {
  try {
    ...
    const backendUrl = new URL(`${modulexServerUrl}/integrations/service-keys`)
    ...
    if (!response.ok) {
      const message = await getApiErrorMessage(response, 'Failed to save service key')
