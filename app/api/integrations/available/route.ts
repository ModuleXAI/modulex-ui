...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendUrl = new URL(`${modulexServerUrl}/integrations/available`)
    ...
    if (!response.ok) {
      return NextResponse.json({ error: 'Failed to fetch available integrations' }, { status: response.status })
    }

    const data = await response.json()
    return NextResponse.json(data)
  } catch (error) {
    console.error('Integrations available API error:', error)
