...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendUrl = new URL(`${modulexServerUrl}/integrations/installed`)
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: 'Failed to fetch installed tools', detail: text }, { status: response.status })
    }

    const data = await response.json()
    return NextResponse.json(data)
  } catch (error) {
    console.error('Integrations installed API error:', error)
